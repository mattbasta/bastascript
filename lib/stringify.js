...
function unexpected(n) {
    ...
    try {
        s += " (" + (n && Object.getOwnPropertyNames(n).toString()) + ")";
    } catch(e) {
        s += " (typeof " + typeof n + ": " + n + ")";
    }
...
function walk(node, callback) {
    ...
    for (var i = 0, key; key = keys[++i];) {
        ...
        if (node[key] instanceof Array) {
            node[key].forEach(function(subelem) {
                if (!isNode(subelem)) return;
                walk(subelem, callback);
            });
            continue;
        }
...
function find(node, nodeType) {
    var results = [];
    walk(node, function(inode) {
        if (inode.type === nodeType) {
            results.push(inode);
        }
    });
    return results;
...
function getDecoratorBase(node, indent) {
    if (node.type === "FunctionDeclaration") {
        var funcIdent = node.id;
        var output;
        if (funcIdent.type === "MemberExpression") {
            output = expr(funcIdent, indent, 3, false);
        } else {
            output = "var " + expr(funcIdent, indent, 3, false);
        }
        output += " = ";
        return output;
    } else {
        return getDecoratorBase(node.body, indent);
    }
}

function getMethodName(node) {
    if (node.type === "MemberExpression") {
        return getMethodName(node.property);
    } else {
        return node.name;
    }
}

function decoratorChain(node, indent) {
    if (node.type === "DecoratedStatement") {
        return expr(node.decorator, indent, 3, true) +
            "(" + decoratorChain(node.body, indent) + ")";
    } else {
        var funcHead = "function" + (node.body.generator ? "*" : "");
        var name = getMethodName(node.id);
        return wrapExpr(functionDeclaration(funcHead, name, node, indent),
                        0, 3);
    }
}
...
function functionDeclaration(init, id, n, indent) {
    ...
    if (id !== null) {
        if (typeof id === "string")
            ...
            name = expr(id, '####', 18, false);
    }

    var body = substmt(n.body, indent + (n.later ? indentChar : "")).trimRight();

    if (n.later) {
        var ind = indent + indentChar;
        body = (
            "{\n" +
            ind + "var ___later = [];\n" +
            ind + "var ___output = (function()" +
            body.trimRight() + ").call(this);\n" +
            ind + "while (___later.length) ___later.shift()();\n" +
            ind + "return ___output;\n" +
            "}"
        );
    }
...
function isBadIdentifier(n) {
    return n.type === "Identifier" && !n.name.match(/^[_$A-Za-z][_$A-Za-z0-9]*$/) && !n.name.match(/^___/);
}
...
function expr(n, indent, cprec, noIn) {
    ...
    case "CurryExpression":
        if (!n.arguments.length) {
            ...
            if (n.callee && n.callee.type !== 'MemberExpression') {
                return expr(n.callee, indent, 17, false);

            // foo.bar->() === foo.bar.bind(foo)
            } else if (n.callee && n.callee.type === 'MemberExpression') {
                return expr(n.callee, indent, 17, false) + '.bind(' + expr(n.callee.object, indent, 17, false) + ')';
            }
        }
        if (n.callee && n.callee.type === 'MemberExpression' && n.callee.computed) {
            return wrapExpr(
                '(function(___base,___member){return function(){return ___base[___member].apply(___base,[' +
                values(n.arguments, function (x){return expr(x, '####', 18, false)}).join(",") +
                '].concat(Array.prototype.slice.call(arguments)));};})' +
                '(' + expr(n.callee.object, indent, 17, false) + ',' + expr(n.callee.property, indent, 17, false) + ')',
                cprec, 3);
        } else if (n.callee && n.callee.type === 'MemberExpression') {
            return wrapExpr(
                '(function(___base){return function(){return ___base.' + expr(n.callee.property, indent, 17, false) + '.apply(arguments[0],[' +
                values(n.arguments, function (x){return expr(x, '####', 18, false)}).join(",") +
                '].concat(Array.prototype.slice.call(arguments)));};})' +
                '(' + expr(n.callee.object, indent, 17, false) + ')',
                cprec, 3);
        } else {
            return wrapExpr('function(){return ' + expr(n.callee, indent, 17, false) + '.apply(this,[' +
                            values(n.arguments, function (x){return expr(x, '####', 18, false)}).join(",") +
                            '].concat(Array.prototype.slice.call(arguments)));}',
                            cprec, 3);
        }
    ...
    case "AssignmentExpression":
        if (n.operator === '.=') {
            return wrapExpr(expr(n.left, indent, 3, noIn) + ' = ' +
                            expr(n.left, indent, 3, noIn) + '.' +
                            expr(n.right, indent, 2, noIn), cprec, 3);
        } else if (n.operator === '=->') {
            n.operator = '=';
            n.right = {
    ...
    case "FunctionExpression":
        return wrapExpr(functionDeclaration("function" + (n.generator ? "*" : ""), n.id, n, indent),
    ...
    case "ArrowFunction":
        var s = "function" + params(n.params, indent);
        if (n.body.type === 'BlockStatement') {
            s += substmt(n.body, indent).trimRight();
        } else {
            s += " {return " + expr(n.body, indent, 0, false) + ";}";
        }
        if (n.body.type === 'ThisExpression' || find(n.body, 'ThisExpression').length) {
            s += ".bind(this)";
        }
        return wrapExpr(s, cprec, 3);
...
function sourceElement(n, indent) {
    ...
    case "LaterStatement":
        var suffix = '';
        if (find(n.argument, 'ThisExpression').length) {
            suffix = '.bind(this)';
        }
        return indent + "___later.push(function() {\n" +
            stmt(n.argument, indent + indentChar) +
            indent + "}" + suffix + ");\n";

    case "YadaYadaStatement":
        return indent + "throw new Error('Not Implemented');\n";
    ...
    case "DecoratedStatement":
        return getDecoratorBase(n.body, indent) +
            decoratorChain(n, indent) + ";\n";

    case "FunctionDeclaration":
        var functionHead = "function" + (n.generator ? "*" : "");
        if (n.id.type === "MemberExpression") {
            return expr(n.id, indent, 3, false) + " = " +
                wrapExpr(functionDeclaration(functionHead, getMethodName(n.id), n, indent),
                         0, n.expression ? 3 : 19) + ";\n";
        } else {
            return (indent +
                    functionDeclaration(functionHead, n.id, n, indent) +
                    (n.expression ? ";\n" : "\n"));
        }
...
function stringify(n, newIndentChar, noHeader) {
    if (n.type != "Program")
        throw new TypeError("argument must be a Program parse node");
    if (newIndentChar) indentChar = newIndentChar;
    var header = "";
    if (!noHeader)
        header = "/* generated by compiling bastascript */\n";
    return header +
        values(n.body, function (x){return sourceElement(x, "")}).join("");
}
