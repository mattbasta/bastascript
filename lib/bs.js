...
parser.yy.parseArrowParams = function(nodes) {
    if (nodes.type === 'Identifier') {
        return [nodes];
    } else if (nodes.type === 'SequenceExpression') {
        return nodex.expressions.map(function(expr) {
            if (expr.type !== 'Identifier') {
                throw new SyntaxError('Unexpected parameter: expected Identifier, got ' + expr.type);
            }
            return expr;
        });
    } else
        throw new SyntaxError('Unexpected arrow function parameter: ' + nodes.type);
};

parser.yy.isLater = false;
